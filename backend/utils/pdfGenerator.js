...
const { convertCurrencyAmount, formatCurrencyAmount } = require('./currency');
...
exports.generateInvoicePDF = async (invoice, user) => {
  return new Promise((resolve, reject) => {
    ...
    const currencyRegion = invoice.currency || invoice.region || 'usd';
    ...
    invoice.items.forEach((item, index) => {
      ...
      doc.text(
        formatCurrencyAmount(
          convertCurrencyAmount(item.unitPrice, sourceCurrency, currencyRegion),
          currencyRegion
        ),
        400,
        y
      ...
      doc.text(
        formatCurrencyAmount(
          convertCurrencyAmount(item.total, sourceCurrency, currencyRegion),
          currencyRegion
        ),
        500,
        y
    ...
    doc.fontSize(12).text(
      `Total Amount: ${formatCurrencyAmount(convertCurrencyAmount(invoice.amount, sourceCurrency, currencyRegion), currencyRegion)}`,
      { align: 'right' }
    );
